...
export class IFlowClient {
  ...
  private updateIFlowCliApiConfig(): void {
    try {
      ...
      this.log(`Failed to update iFlow CLI API config: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
  ...
  async checkAvailability(): Promise<{ version: string | null; diagnostics: string }> {
    ...
    try {
      ...
      if (errorMsg.includes('Failed to connect') || errorMsg.includes('WebSocket')) {
        ...
        suggestions.push('2. 手动启动 CLI 查看输出: node "C:\\Users\\76578\\AppData\\Roaming\\npm\\node_modules\\@iflow-ai\\iflow-cli\\bundle\\entry.js" --experimental-acp --port 8090');
